...
class AddModifyCategoryViewModel: BaseViewModel {
    ...
    func addNewCategory(_ model: CategoryModel, categoryType: CategoryType) async -> ResponseModel {
    ...
    func modifyCategory(_ model: CategoryModel, categoryType: CategoryType) async -> ResponseModel {
        if model.name.isEmptyOrWhitespace {
            return ResponseModel(.error, Errors.emptySpaces.localizedDescription)
        }
